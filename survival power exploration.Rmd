...
```{r exp, results='hide',eval=TRUE , echo=TRUE}
  ...
  lambda  <-runif(1,1,3)                       # random, this will be a constant hazard rate
  lambda2 <-runif(1,1,3)                       # random, second curve rate
  
  #new
  A  <-runif(1,1,3)                       # random, this will be a constant hazard rate
  B  <-runif(1,1,3)                       # random, second curve rate
  lamda   <- min(A,B)           # best survival
  lambda2 <- max(A,B)           # worst surivial red curve
  #end new
  
  end <- ceiling(-(log(1-.999)/ lambda))       # for plotting 99th percentile
  s <- seq(0,end, length.out = end+1)          # reasonable x axis
  #p <- 0.5                                    # choose survival prop
  p <- runif(1)                                # random survival prop
  ms <- -log(1-p)/lambda                       # survival time  for survival prop p
  ...
  curve(zp(x, shape=1, scale=1/lambda), from=0, to=end, 
         main=paste0("A change in survival probability at fixed time:\n S(t) for exponential rates ",
                     formatz2(lambda),         " (red) and ",
                     formatz2(lambda2),        " (blue): HR= ",
                     formatz2(lambda/lambda2), "\nMortalities at time ",
                     formatz4(ms),             ": ",
                     formatz2(p*100),          "% (red) and ", 
                     formatz2((1-mort1)*100 ), "% (blue)" ), 
                     cex.main = .8,
                     ylab='Survival probability', xlab='', col="red", 
  ...
  #~~~~~~~~~~~~~~~~~~
  text(x = end*.6, y = .95, #(1-p)*1.1, #.7,                # Text with different color & size
       ...
       cex = .8)
  #~~~~~~~~~~~~~~~~~~
  text(x = end*.537, y = .875, #mort1*.9, #.5,                # Text with different color & size
       paste0(expression("HR = hazard rate[red] / hazard rate[blue] = "),
              formatz2(lambda),         " / ",
              formatz2(lambda2),        " = ",
              formatz2(lambda/lambda2), ""
              ),
...
# temp code these out
# fact. <-     runif(1) # random % increase in time for survival of this means
# lambda2  <-  lambda/(1+fact.)  # use lambda from previous chunk
# hr    <-     lambda2/lambda
 
 
 # ref.time <- -log(p)/lambda
 # trt.time <- -log(p)/lambda2
 
 # new
 p <- mort1   # copy the blue 
 ref.time <- s1 <- -log(1-p)/lambda
 trt.time <- s2 <- -log(1-p)/lambda2 
 fact. <- perc.chg <- (1+(s1/s2-1))
 ...
 curve(zp(x, shape=1, scale=1/lambda), from=0, to=end, 
       main=paste0(  "A change in survival time fixed survival probability:\n S(t) for exponential rates ",
          formatz2(lambda),    " (blue) and ",
          formatz2(lambda2),   " (red). HR= ",
          formatz2(hr),        " (blue/red)\n A survival probability of ",
          formatz4(p),         " for the reference has survival time ",
          formatz2( ref.time), ". \nA ",
          formatz2(fact.*100) ,"% increase in the survival time results in a survival time of ",
          formatz2(trt.time),  " (red)"),  cex.main = .8,
          ylab='Survival probability',xlab='', 
          col="blue",
          sub= "Time\nIn the case of exponential distributions, the reciprocal of the ratio of medians (or any other quantile) gives e(b). \nFor example, if we want to detect a 50% increase in median survival time, we would set e(Beta) = 2/3" , cex.sub=.8)
 
   text(x = end*.6, y = .95, #p*1.2,              # Text with different color & size
       paste0("HR at survival quantile ",formatz4(p),", ratio of survivals blue / red = "
              ,formatz2(ref.time),             " / ",
              formatz2(trt.time),              " = ", 
  ...
  #~~~~~~~~~~~~~~~~~~
  text(x = end*.52, y = .875, #mort1*.9, #.5,                # Text with different color & size
       paste0(expression("HR = hazard rate[red] / hazard rate[blue] = "),
              formatz2(lambda2)," / ",formatz2(lambda)," = "
              ,formatz2(lambda2/lambda),""
              ),
       col = "black",#1b98e0",
             cex = .8)
  #~~~~~~~~~~~~~~~~~~~
 abline(v= -log(p)/lambda, col='blue')  
 abline(h=p , col='black' , lty=2)
 abline(v= -log(p)/lambda2 , col='red')   
...
# This function is useful for creating d1 an input to Hmisc cpower
