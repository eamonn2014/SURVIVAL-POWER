...
# Exponential distributions, a change in mortality at fixed time
...
```{r exp, results='hide',eval=TRUE , echo=TRUE}
  ...
  curve(zp(x, shape=1, scale=1/lambda), from=0, to=end, 
        ...
        ylab='Survival probability', xlab='', col="red", sub= "Time\nIn the case of exponential distributions, the reciprocal of the ratio of medians (or any other quantile) gives e(b). \nFor example, if we want to detect a 50% increase in median survival time, we would set e(Beta) = 2/3" ,
          cex.sub=.9)
  ...
  text(x = end*.6, y = (1-p)*1.1, #.7,                # Text with different color & size
       paste0("HR = log(Survival)[2]/log(Survival)[1] = log("
              ,formatz2(1-p),
              ") / log(",
              formatz2(mort1),
              ") = ", 
  ...
  text(x = end*.6, y = mort1*.9, #.5,                # Text with different color & size
       paste0(expression("HR= hazard rate[1] / hazard rate[2] = "),formatz2(lambda)," / ",formatz2(lambda2)," = "
...
# Exponential distributions, a change in survival time fixed survival
...
```{r exp2, results='hide',eval=TRUE , echo=TRUE}
 ...
 curve(zp(x, shape=1, scale=1/lambda), from=0, to=end, 
       main=paste0("S(t) for exponential rates ",formatz2(lambda)," (red) and ",formatz2(lambda2), " (blue). HR= ",formatz2(hr),".\n A survival probability of ",formatz4(p)," for the reference has survival time ",
                   formatz2( ref.time),". \nA ",formatz2(fact.*100) ,"% increase in the survival time results in a survival time of ",formatz2(trt.time)," (blue)"),  cex.main = .8,
       ylab='Survival probability', #xlab='Time', 
       col="red",
        sub= "Time\nIn the case of exponential distributions, the reciprocal of the ratio of medians (or any other quantile) gives e(b). \nFor example, if we want to detect a 50% increase in median survival time, we would set e(Beta) = 2/3" )
 
 
   text(x = end*.6, y = p*1.2,              # Text with different color & size
       paste0("HR = Quantile ",formatz4(p),", ratio of survivals = "
              ,formatz2(ref.time),
              " / ",
              formatz2(trt.time),
              " = ", 
